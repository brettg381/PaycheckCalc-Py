...
from payroll.apps.registration.backends import get_backend
...
# Async Register Methods
def registration(request):
    backend = get_backend('payroll.apps.registration.backends.default.DefaultBackend')
    form_class = backend.get_form_class('payroll.apps.registration.forms.PayrollRegistrationForm')

    user_data={'status':0, 'error:':None}
    if request.method == 'POST':
        form = PayrollRegistrationForm(data=request.POST, files=request.FILES)
        if form.is_valid():
            try:            
                new_user = backend.register(request, **form.cleaned_data)
                ...
                user_data['error'] = 'Email already exists'
...
def login(request):
    ...
    user_data={'status':user_status, 'object:':None}
    return HttpResponse(json.dumps(user_data), mimetype="text/javascript")
